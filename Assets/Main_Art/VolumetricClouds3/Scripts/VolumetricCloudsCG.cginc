...
f4 _PerlinNormalMap_ST, _CoverageMap_ST, _CoverageChannel;
...
inline f3 RaySphereIntersection(f3 spherePos, f1 sphereRadius, f3 rayPos, f3 rayDirection, f1 faceSign, out bool oob)
{
    f3 sToR = rayPos - spherePos;
    f1 dPerp = dot(sToR, rayDirection);
    f1 c = dot(sToR, sToR) - sphereRadius * sphereRadius;
    
    oob = c > 0.0f && dPerp > 0.0f ? true : false;
	
    f1 discr = dPerp*dPerp - c;
    
    oob = discr < 0.0f ? true : oob;
    
    f1 t = -dPerp + sqrt(discr) * faceSign;
...
{
    f1 rsDist = distance(spherePos, rayPos);
    // Fold branches
	int select = rsDist < sphereRadius - sphereDepth ? 1 :
	             rsDist > sphereRadius + sphereDepth ? -1 : 0;
    
    f3 output = RaySphereIntersection(spherePos, sphereRadius - sphereDepth * select, rayPos, rayDirection, select, /*out*/oob);
    // select == 0 => rayPos wihtin the cloud volume
    output = select == 0 ? rayPos : output;
    oob = oob && select != 0;
	return output;
}
...
#else
	oob = false;
	if(samplePos.y < CloudUpperBounds && samplePos.y > CloudLowerBounds)
	    return samplePos;
	f3 offsetOrthoToBounds = f3(samplePos.x, samplePos.y > CloudUpperBounds ? CloudUpperBounds : CloudLowerBounds, samplePos.z) - samplePos;
...
#if _COVERAGEMAP
	f1 coverageMap = length(tex2Dlod(_CoverageMap, f4((UV+_CoverageMap_ST.zw)/_CoverageMap_ST.xy, 0, lod)) * _CoverageChannel);
...
// Doesn't reproduce the same color over all unity versions, later ones seems to be more faithful to the skybox's color, blame unity.
...
inline void PushSampleForward(inout f3 samplePos, inout f1 distanceTravelled, f3 dir, f1 signedDist)
{
    ...
    #if( _NEW_SHADING && _SPHERE_MAPPED )
        if( distance(samplePos, _CloudSpherePosition) < CloudLowerBounds )
        {
            bool oob;
            f3 previous = samplePos;
            samplePos = RaySphereIntersection(_CloudSpherePosition, CloudCenterBounds - _CloudTransform.y, samplePos, dir, 1, /*out*/oob);
            distanceTravelled += distance(previous, samplePos);
        }
    #endif
}
