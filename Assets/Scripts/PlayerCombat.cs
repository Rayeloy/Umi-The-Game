...
public class PlayerCombat : MonoBehaviour {
    ...
    public void KonoUpdate()
    {
        ...
        if (!myPlayerMovement.noInput && !myPlayerMovement.inWater && attackStg == attackStage.ready)
        {
            ...
            if (LTPulsado && !RTPulsado && myPlayerMovement.Actions.Boost)//Input.GetButtonDown(myPlayerMovement.contName + "RB"))
            {
                RTPulsado = true;
        ...
        if (myPlayerMovement.Actions.Boost)//Input.GetButtonUp(myPlayerMovement.contName + "RB"))
        {
            RTPulsado = false;
        }

        if (myPlayerMovement.Actions.Aim.IsPressed/*Input.GetButtonDown(myPlayerMovement.contName + "LB")*/ && !LTPulsado)
        {
            print("startAiming");
            LTPulsado = true;
            StartAiming();
