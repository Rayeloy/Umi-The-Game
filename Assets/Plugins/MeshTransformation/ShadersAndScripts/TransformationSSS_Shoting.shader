Shader "Transformation/SSS_Shoting"
{
	Properties
	{
		...
		_intensityHoles ("intensityHoles", Float) = 1
	...
	uint id : SV_VertexID;
		...
		void vertexDataFunc( inout appdata__ v , out Input o )
		{
			...
			float p = 0;
			#ifdef SHADER_API_D3D11
			p =  _colors[v.id] * tex2Dlod( _TransformationMask, float4( uvMask_218, 0, 0.0) ).r * _intensityHoles;

			#endif
			mask__ = clamp( _IntensityMask,0,_MaxIntensityMask) ; 
			float temp_output_93_0 = (_Transformation * mask__ * p);
			float3 vvvv = 0;
			float3 nn = 0;
			float3 delta_;
			delta_ = posObj;
		...
		void surf( Input i , inout SurfaceOutputStandardSpecularCustom o )
		{
			...
			p = tex2D( _TransformationMask, uvMask_218 ) * _intensityHoles * i.vertexColor;
			...
			float3 SSS = ( lerpResult80 * lerpResult192 * clampResult85 ).rgb;
			#if UNITY_COLORSPACE_GAMMA
