...
Shader "Transformation/Simple LWRP"
{
	Properties
	{
		...
		[Toggle]_InvertX("InvertX", Float) = 0
	...
	SubShader
	{
		...
		uniform float _Cutoff = 0.5;
			...
			GraphVertexOutput vert (GraphVertexInput v)
			{
				...
				float2 uv230 = v.ase_texcoord3.xy * temp_cast_9 + float2( 0,0 );
				float2 uvMask_218 = lerp(lerp(lerp(lerp(uv231,uv231,_uv0Mask),uv232,_uv1Mask),uv233,_uv2Mask),uv230,_uv3Mask);
			...
			half4 frag (GraphVertexOutput IN ) : SV_Target
		    {
				...
				float2 uvDefault214 = lerp(lerp(lerp(lerp(uv198,uv198,_uv0),uv199,_uv1),uv200,_uv2),uv201,_uv3);
				float2 uvDefault2217 = lerp(lerp(lerp(lerp(uv198,uv198,_uv0Model2),uv199,_uv1Model2),uv200,_uv2Model2),uv201,_uv3Model2);
...
float2 uv_Noise = uvDefault214 * _Noise_ST.xy + _Noise_ST.zw;

			_Holes = lerp (0,_Holes,_Transformation);
			float lerpResult18 = lerp( mask177 , tex2D( _Noise, uv_Noise ).r , _Holes);
			#if UNITY_COLORSPACE_GAMMA
            lerpResult18 = FastSRGBToLinear(lerpResult18);

			
            #endif
			clip( lerpResult18.r - _Cutoff );
float AlphaClipThreshold = lerpResult18.r - _Cutoff;
				InputData inputData;
	...
	//		uniform float4 _ShadowBias;
			...
			GraphVertexOutput vert (GraphVertexInput v)
			{
				...
				float2 temp_cast_3 = (_TillingMask).xx;
				float2 temp_cast_4 = (_OffsetMask).xx;
				float2 uv231 = v.ase_texcoord * temp_cast_3 + temp_cast_4;
				float2 temp_cast_5 = (_TillingMask).xx;
				float2 temp_cast_6 = (_OffsetMask).xx;
				float2 uv232 = v.ase_texcoord1 * temp_cast_5 + temp_cast_6;
				float2 temp_cast_7 = (_TillingMask).xx;
				float2 temp_cast_8 = (_OffsetMask).xx;
				float2 uv233 = v.ase_texcoord2 * temp_cast_7 + temp_cast_8;
				float2 temp_cast_9 = (_TillingMask).xx;
				float2 uv230 = v.ase_texcoord3 * temp_cast_9 + float2( 0,0 );
				float2 uvMask_218 = lerp(lerp(lerp(lerp(uv231,uv231,_uv0Mask),uv232,_uv1Mask),uv233,_uv2Mask),uv230,_uv3Mask);
				float mask177 = ( ( clampResult175 * _Transformation ) * tex2Dlod( _TransformationMask, float4( uvMask_218, 0, 0.0) ).r );
