...
public class HousingGrid : MonoBehaviour
{
    ...
    public bool SpawnFurnitureAt(HousingFurnitureData _furnitureMeta, HousingGridCoordinates spawnCoord, out HousingFurniture _furniture)
    {
        ...
        else
        {
            ...
            {
                currentFurniture = null;
                Debug.LogError("SpawnFurnitureAt: Can't select the slot " + _furniture.currentAnchorGridPos.printString + " with the furniture " + _furniture.name);
                return false;
            }
        }
        ...
        for (int k = 0; k < _furniture.height; k++)//for every furniture level
        {
            for (int i = 0; i < _furniture.depth; i++)//for every furniture row
            {
                for (int j = 0; j < _furniture.width; j++)//for every furniture slot
                {
                    ...
                    {
                        Debug.LogError("CanFurnitureFit: " + currentCheckCoord.printString + " is not a valid grid coordinate");
                        return false;
                    }
                    ...
                    switch (_furniture.furnitureMeta.furnitureType)
                    {
                        case FurnitureType.Floor:
                            ...
                            {
                                validSlot = true;
                                if (k == 0 && !baseFound && (OnFloor(currentCheckCoord) || OnTopOfPile(currentCheckCoord)))
                                {
                                    baseFound = true;
                                }
                            }
                        ...
                        case FurnitureType.Wall:
                            if ((slot != null && slot.free && slot.CanPlaceWallFurniture(_furniture)))
                            {
                                validSlot = true;
                            }
                            break;
                    ...
                    if (!validSlot)
                    {
                        Debug.LogError("CanFurnitureFit: Can't place furniture " + _furniture.furnitureMeta.name + " with anchor at " + _furniture.currentAnchorGridPos.printString +
    "; conflict at " + currentCheckCoord.printString + "; k = " + k + "; i = " + i + "; j = " + j);
                        return false;
                    }
                    else
                    {
                        Debug.Log("CanFurnitureFit: found good selectable slot at " + currentCheckCoord.printString);
                    }
                }
            }
        }
        ...
        if (_furniture.furnitureMeta.furnitureType == FurnitureType.Floor || _furniture.furnitureMeta.furnitureType == FurnitureType.Floor_Small)
        {
            if (!baseFound)
            {
                Debug.LogError("CanFurnitureFit: not even 1 base found");
                return false;
            }
        }
        result = true;
    ...
    public bool PlaceFurniture()
    {
        ...
        if (currentFurniture == null)
        {
            Debug.LogError("Can't place furniture because currentFurniture == null.");
            return false;
        }
        ...
        else
        {
            ...
            if (!result)
            {
                Debug.LogError("PlaceFurniture : Can't place furniture here because 1 or more slots are not valid.");
                return false;
            }
            ...
            for (int k = 0; k < currentFurniture.height; k++)//for every furniture level
            {
                for (int i = 0; i < currentFurniture.depth; i++)//for every furniture row
                {
                    ...
                    {
                        HousingGridCoordinates currentCheckCoord = currentFurniture.GetGridCoord(new HousingGridCoordinates(k, i, j));
                        HousingSlot currentSlot = GetSlotAt(currentCheckCoord);
                        Debug.Log("PlaceFurniture: Going to set furniture at " + currentCheckCoord.printString + "; furniture's k = " + k + "; i = " + i + "; j = " + j);
                        if (!currentSlot.SetFurniture(currentFurniture))
                        {
                            Debug.LogError("PlaceFurniture: can't set furniture, This should NOT be happening because we already checked before");
                            return false;
                        }
                    }
                }
        ...
        if (result)
        {
            //Check if furniture under
            for (int i = 0; i < currentFurniture.depth; i++)//for every furniture row
            {
                for (int j = 0; j < currentFurniture.width; j++)//for every furniture slot
                {
                    HousingGridCoordinates currentCheckCoord = currentFurniture.GetGridCoord(new HousingGridCoordinates(0, i, j));
                    currentCheckCoord.y--;
                    if (IsCoordValid(currentCheckCoord))
                    {
                        ...
                        {
                            if (!currentFurniture.furnitureUnder.Contains(underSlot.myFurniture))
                            {
                                currentFurniture.furnitureUnder.Add(underSlot.myFurniture);
                                Debug.Log("PlaceFurniture: Adding underFurniture");
                                if (!underSlot.myFurniture.smallFurnitureOn.Contains(currentFurniture))
                                    underSlot.myFurniture.smallFurnitureOn.Add(currentFurniture);
                                else Debug.LogError("PlaceFurniture: This should not be happening. the furniture " + currentFurniture.name +
    " is already in the smallFurnitureOn list of " + underSlot.myFurniture.name + " .");
                            }
                            else
                            {
                                Debug.LogError("PlaceFurniture: This should not be happening. the furniture " + underSlot.myFurniture.name +
                                    " is already in the furnitureUnder list of " + currentFurniture.name + " .");
                            }

                        }
                        else
                        {
                            Debug.Log("PlaceFurniture: This coord " + currentCheckCoord.printString + " should be floor. Check it.");
                        }
                    }
                    else
                    {
                        Debug.LogError("PlaceFurniture: Coord not valid ->" + currentCheckCoord.printString + "; This may be normal (check code)");
                    }
                }
            }
    ...
    bool PlaceWallFurniture()
    {
        ...
        if (currentFurniture == null)
        {
            Debug.LogError("PlaceWallFurniture: Can't place furniture because currentFurniture == null. This message should not happen because this function " +
        ...
        for (int k = 0; k < currentFurniture.height; k++)//for every furniture level
        {
            for (int i = 0; i < currentFurniture.depth; i++)//for every furniture row
            {
                for (int j = 0; j < currentFurniture.width; j++)//for every furniture slot
                {
                    HousingGridCoordinates currentCheckCoord = currentFurniture.GetGridCoord(new HousingGridCoordinates(k, i, j));
                    HousingSlot currentSlot = GetSlotAt(currentCheckCoord);
                    Debug.Log("PlaceWallFurniture: Going to set furniture at " + currentCheckCoord.printString + "; furniture's k = " + k + "; i = " + i + "; j = " + j);
                    if (!currentSlot.SetFurniture(currentFurniture))
                    {
                        Debug.LogError("PlaceWallFurniture: can't set furniture, This should NOT be happening because we already checked before");
                        return false;
                    }
                }
            }
    ...
    {
        Debug.LogWarning("START CHECK IF FURNITURE SLOT CAN BE PLACED");
        bool result = false;
        if (IsCoordValid(coord))
        {
            ...
            if (currentSlot != null && currentSlot.free)
            {
                ...
                {
                    case FurnitureType.Floor:
                        if ((furnitureBase && OnFloor(coord)) || !furnitureBase)
                            result = true;
                        break;
                    case FurnitureType.Floor_Small:
                        if ((furnitureBase && (OnFloor(coord) || OnTopOfPile(coord))) || !furnitureBase)
                            result = true;
                        break;
                    case FurnitureType.Wall:
                        if (CanSelectWallSlot(coord))
                            result = true;
                        break;
                }
            }
            if (!result)
            {
                Debug.LogError("Can't place furniture Slot of " + _furniture.furnitureMeta.name + "  at " + coord.printString);
                return false;
            }
        }
        return result;
    ...
    public bool PickFurniture()
    {
        ...
        {
            Debug.LogError("Can't pick furniture because currentFurniture != null or highlightedFurniture == null.");
            return false;
        }
        if (highlightedFurniture.smallFurnitureOn.Count > 0)
        {
            Debug.LogError("Can't pick furniture because there is a small furniture on it.");
            return false;
        }
        ...
        for (int k = 0; k < highlightedFurniture.height; k++)//for every furniture level
        {
            for (int i = 0; i < highlightedFurniture.depth; i++)//for every furniture row
            {
                ...
                {
                    HousingGridCoordinates currentCheckCoord = highlightedFurniture.GetGridCoord(new HousingGridCoordinates(k, i, j));
                    HousingSlot currentSlot = GetSlotAt(currentCheckCoord);
                    if (!currentSlot.PickFurniture(highlightedFurniture))
                    {
                        Debug.LogError("PickFurniture: can't pick furniture at " + currentCheckCoord.printString + "; furniture's k = " + k + "; i = " + i + "; j = " + j);
                        return false;
                    }
                }
            }
        ...
        {
            Debug.LogError("PickFurniture: Can't highlight furniture");
            return false;
        }

        if (!HighlightSlot(currentSlotCoord, 2))
        {
            Debug.LogError("PickFurniture: Can't highlight anchor's slot");
            return false;
        }
        while (currentFurniture.furnitureUnder.Count > 0)
        {
            currentFurniture.furnitureUnder[0].smallFurnitureOn.Remove(currentFurniture);
            currentFurniture.furnitureUnder.RemoveAt(0);
        }
    ...
    bool HighlightSlotFull(HousingGridCoordinates coord, int state)//0 == good; 1 == bad; 2 == pointer; 3 == placed
    {
        ...
        if (slot == null)
        {
            Debug.LogError("HighlightSlot: Invalid slot to highlight");
            return false;
        }
        ...
        if (currentFurniture == null)
        {
            ...
            {
                Debug.LogError("Couldn't highlight slot " + coord.printString);
                return false;
            }
        }
        else
        {
            Debug.LogError("HighlightSlotFull: TO DO");
        }
        ...
        for (int k = 0; k < _furniture.height; k++)//for every furniture level
        {
            for (int i = 0; i < _furniture.depth; i++)//for every furniture row
            {
                for (int j = 0; j < _furniture.width; j++)//for every furniture slot
                {
                    HousingGridCoordinates currentCheckCoord = _furniture.GetGridCoord(new HousingGridCoordinates(k, i, j));
                    HousingSlot currentSlot = GetSlotAt(currentCheckCoord);
                    Debug.Log("highlighting furniture slot " + currentCheckCoord.printString);
    ...
    public bool StopHighlightPlacedFurniture()
    {
        ...
        {
            Debug.LogWarning("StopHighlightPlacedFurniture: can't stop hightlighting this furniture.");
            return false;
        }
        for (int k = 0; k < highlightedFurniture.height; k++)//for every furniture level
        {
            for (int i = 0; i < highlightedFurniture.depth; i++)//for every furniture row
            {
                for (int j = 0; j < highlightedFurniture.width; j++)//for every furniture slot
                {
                    ...
                    if (slot != null && slot.HasThisFurniture(highlightedFurniture))
                    {
                        ...
                        {
                            Debug.LogWarning("StopHighlightPlacedFurniture: Can't Highlight Furniture at slot " + currentCheckCoord.printString);
                            return false;
                        }
                    }
                }
    ...
    bool HighlightPickedFurniture()
    {
        ...
        if (currentFurniture == null)
        {
            Debug.LogError("HighlightPickedFurniture: there is not picked furniture");
            return false;
        }
        for (int k = 0; k < currentFurniture.height; k++)//for every furniture level
        {
            for (int i = 0; i < currentFurniture.depth; i++)//for every furniture row
            {
                for (int j = 0; j < currentFurniture.width; j++)//for every furniture slot
                {
                    ...
                    if (validSlot) Debug.Log("HighlightPickedFurniture:  Found a good placeable slot at " + currentCheckCoord.printString);
                    int value = validSlot ? 0 : 1;
                    if (!HighlightSlot(currentCheckCoord, value))
                    {
                        Debug.LogError("Can't highlight the slot " + currentCheckCoord.printString);
                        return false;
                    }
                }
            }
    ...
    public bool StopHighlightPickedFurniture(HousingGridCoordinates oldAnchorCoord)
    {
        ...
        if (currentFurniture == null)
        {
            Debug.LogWarning("StopHighlightPickedFurniture: can't stop hightlighting this furniture.");
            return false;
        }
        if (highlightedFurniture == null)
        {
        ...
        for (int k = 0; k < highlightedFurniture.height; k++)//for every furniture level
        {
            for (int i = 0; i < highlightedFurniture.depth; i++)//for every furniture row
            {
                for (int j = 0; j < highlightedFurniture.width; j++)//for every furniture slot
                {
                    HousingGridCoordinates currentCheckCoord = highlightedFurniture.GetGridCoord(new HousingGridCoordinates(k, i, j), oldAnchorCoord);
                    HousingSlot slot = GetSlotAt(currentCheckCoord);
                    Debug.Log("StopHighlightPickedFurniture: k = " + k + "; i = " + i + "; j = " + j + "; oldAnchorCoord = " + oldAnchorCoord.printString);
                    if (slot != null)
                    {
                        if (!StopHighlightSlot(currentCheckCoord))
                        {
                            Debug.LogWarning("StopHighlightPickedFurniture: Can't Stop Highlight Furniture at slot " + currentCheckCoord.printString);
                            return false;
                        }
                    }
                    else
                    {
                        Debug.LogError("StopHighLightPickedFurniture: This should not be happening. This means we had the furniture in a non existing slot in the last movement!");
                    }
                }
            }
        }
    ...
    bool InPile(HousingGridCoordinates coord)
    {
        if (!IsCoordValid(coord))
        {
            Debug.LogError("InPile -> Invalid Coords: " + coord.printString);
            return false;
        }
        HousingSlot slot = GetSlotAt(coord);
        HousingSlot downSlot = null;
    ...
    bool TopOfPile(HousingGridCoordinates coord)
    {
        if (!IsCoordValid(coord))
        {
            Debug.LogError("TopOfPile -> Invalid Coords: " + coord.printString);
            return false;
        }
        HousingSlot slot = GetSlotAt(coord);
        HousingSlot upSlot = null;
    ...
    bool CanSelectSlot(HousingGridCoordinates coord)
    {
        ...
        {
            Debug.LogError("CanSelectSlot -> Invalid Coords: " + coord.printString);
            return false;
        }
    ...
    bool CanSelectWallSlot(HousingGridCoordinates coord)
    {
        if (!IsCoordValid(coord))
        {
            Debug.LogError("CanSelectWallSlot -> Invalid Coords: " + coord.printString);
            return false;
        }
        HousingSlot slot = GetSlotAt(coord);

        return slot != null && (slot.hasAnyWall);
    }
    ...
    public bool SelectSlotAt(HousingGridCoordinates coord)
    {
        ...
        if (currentFurniture == null)
        {
            ...
            else
            {
                for (int k = coord.y; k < height && !foundValid; k++)//Check upwards
                {
                    ...
                    if (CanSelectSlot(checkCoord))
                    {
                        ...
                        coord.y = k;
                        foundValid = true;
                        Debug.Log("Found good Selectable slot at " + coord.printString);
                    }
                }
                if (!foundValid)
                {
            ...
            if (!foundValid)
            {
                Debug.LogError("SelectSlotAt: couldn't find a valid slot to select");
                return false;
            }
            ...
            {
                Debug.LogError("SelectSlotAt: Can't highlight slot full");
                return false;
            }
        }
        #endregion
        ...
        else// select a slot while having a furniture picked. Always move anchor and then check furniture around it
        {
            ...
            {
                HousingGridCoordinates checkCoord = new HousingGridCoordinates(k, coord.z, coord.x);
                currentFurniture.currentAnchorGridPos = checkCoord;
                if (CanFurnitureFit(currentFurniture))
                {
                    //stickToWall = GetSlotAt(checkCoord).hasAnyWall;
                    coord.y = k;
                    foundValid = true;
                    Debug.Log("Found good Selectable slot at anchor " + coord.printString);
                }
            }
            if (!foundValid)
            {
            ...
            if (!foundValid)
            {
                Debug.LogError("SelectSlotAt: couldn't find valid slots to select with furniture");
                return false;
            }
            ...
            {
                Debug.LogError("SelectSlotAt: Can't highlight picked furniture");
                return false;
            }
            ...
            {
                Debug.LogError("SelectSlotAt: Can't highlight anchor's slot");
                return false;
            }
        }
        #endregion
    ...
    public bool SelectWallSlotAt(HousingGridCoordinates coord, bool goingDown = false)
    {
        ...
        if (currentFurniture == null)
        {
            ...
            if (!foundValid)
            {
                Debug.LogError("SelectWallSlotAt: Couldn't find valid slot going " + (goingDown ? "down" : "up"));
                return false;
            }
    ...
    public bool MoveSelectUp()
    {
        ...
        if (!result)
        {
            Debug.LogError("Can't MoveSelectUp because we can't select the slot " + coord.printString);
            return false;
        }
    ...
    public bool MoveSelectDown()
    {
        ...
        if (!result)
        {
            Debug.LogError("Can't MoveSelectDown because we can't select the slot " + coord.printString);
            return false;
        }
    ...
    public bool RotateFurniture(bool clockwise)
    {
        ...
        if (currentFurniture != null)
        {
            ...
            FurnitureLevel[] oldSpaces = currentFurniture.currentSpaces;
            HousingGridCoordinates oldAnchor = currentFurniture.anchor;
            ...
            {
                Debug.LogError("RotateFurniture: can't Select furniture at " + currentSlotCoord.printString);
                currentFurniture.currentSpaces = oldSpaces;
                currentFurniture.anchor = oldAnchor;
                currentFurniture.ResetRotationBools();
                currentFurniture.PrintSpaces();
                return false;
            }
