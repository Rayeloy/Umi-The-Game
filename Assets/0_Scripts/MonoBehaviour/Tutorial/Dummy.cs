...
public class Dummy : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider col)
    {
        switch (col.tag)
        {
            case "Water":
                ...
                float minHeight = transform.position.y - (GetComponent<CapsuleCollider>().bounds.max.y / 4);
                if (minHeight <= waterSurface)
                {
                    EnterWater();
                }
                else
    ...
    public void StartInflatingDummy()
    {
        //animación hinchado
