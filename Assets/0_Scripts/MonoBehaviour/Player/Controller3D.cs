...
public class Controller3D : MonoBehaviour
{
    ...
    [Header(" -- Vertical Floor Edge -- ")]
    public bool showFloorEdgeRays;
    public int floorEdgeHighPrecissionRays = 5;
    float floorEdgeHighPrecissionHorRaySpacing;
    float floorEdgeHighPrecissionVerRaySpacing;
    ...
    public void Move(Vector3 vel)
    {
        ...
        if (!disableAllDebugs) Debug.Log("Middle Vel = " + vel.ToString("F4") + "; CollisionState = " + collisions.collSt + "; wallSlideCollSt = " + collisions.wallSlideCollSt +
    ...
    bool ClimbSlope(ref Vector3 vel, Raycast rayCast, bool wallSlide = false)
    {
        if (!disableAllDebugs) Debug.Log("Start ClimbSlope = " + vel.ToString("F4") + "; CollisionState = " + collisions.collSt + "; wallSlideCollSt = " + collisions.wallSlideCollSt
                 + "; below = " + collisions.below);
        ...
        if (rayCast.vel.y <= climbVel.y)
        {
            ...
            {
                collisions.wallSlideCollSt = CollisionState.climbing;
                collisions.wallSlideSlopeAngle = rayCast.slopeAngle;
            }
            if (!disableAllDebugs) Debug.Log("Finish ClimbSlope = " + vel.ToString("F4") + "; CollisionState = " + collisions.collSt + "; wallSlideCollSt = " + collisions.wallSlideCollSt
                + "; below = " + collisions.below);
        ...
        {
            if (!disableAllDebugs) Debug.Log("Tried to climb, but my vel.y is faster than my climbing.y speed (jumping while climbing?)");
            return false;
        }
    ...
    CollisionState CheckSlopeType(Vector3 vel, Raycast ray)
    {
        ...
        {
            //if (!disableAllDebugs) Debug.LogError("CheckSlopeType Y axis -> climbVel = " + climbVel.ToString("F4") + "; ray.slopeAngle = " + ray.slopeAngle);
            if (climbVel.y > 0 && !IsSlopeAngleAWall(ray.slopeAngle))
            {
                return CollisionState.climbing;
            }
            ...
            else if (collisions.collSt != CollisionState.climbing && collisions.wallSlideCollSt != CollisionState.climbing && ((vel.y < 0 && ray.slopeAngle > maxDescendAngle) || (vel.y > 0 && ray.slopeAngle != 180)))
            {
                return CollisionState.sliping;
            }
            else if ((ray.slopeAngle >= 0 && ray.slopeAngle < maxClimbAngle) || ray.slopeAngle == 180)
            {
                return CollisionState.roof_floor;//FLOOR
            }
            else
            {
    ...
    void HorizontalCollisions(ref Vector3 vel, bool wallSlide)
    {
        ...
        {
            if (!disableAllDebugs) if (wallSlide) Debug.LogWarning("<--------- WALL SLIDE COLLISIONS --------->");
            ...
            if (closestClimbRaycast.hit && closestWallRaycast.hit)
            {
                ...
                {
                    float climb_wall_distDiff = Mathf.Abs(closestClimbRaycast.distance - closestWallRaycast.distance);
    ...
    FirstCollisionWithWallType.climbingAndBackwardsWall : FirstCollisionWithWallType.climbingAndStraightWall;
                    ...
                    else if (collisions.lastCollSt == CollisionState.wall && climb_wall_distDiff <= 0.001)
                    {
                        value = CollisionState.wall;
                        closestRaycast = closestWallRaycast;
                        if (!disableAllDebugs) Debug.LogWarning("SLOPE EDGE PROBLEMATIC: we decided to collide with wall since we were doing it the last frame and the distances are close to 0.");
                    }
                    else if (!disableAllDebugs && collisions.lastCollSt == CollisionState.wall && climb_wall_distDiff > 0.001) Debug.LogError("WHY ARE THE WALL AND THE SLOPE SO FAR AWAY?!?!?!?");
                }
            }
            #endregion
            ...
            switch (value)//con que tipo de objeto colisionamos? pared/cuesta arriba/cuesta abajo
            {
                ...
                case CollisionState.wall:
                    ...
                    #region -- Wall Edges --
                    if (!WallEdgeAll(vel, ref closestRaycast, rows, raysPerRow, wallSlide) && !wallSlide)
                    {
                        collisions.wallEdgeFirstWallAngle = closestRaycast.wallAngle;
                    }
                    #endregion
      ...
      || closestRaycast.wallAngle == collisions.wallEdgeNewWallAngle))
                ...
                case CollisionState.climbing:
                    ...
                    if (closestRaycast.distance < 0)//SI ESTAMOS METIDOS DENTRO DEL MURO POR ALGUN MOTIVO
                    {
                        if (!disableAllDebugs) Debug.LogWarning("WE GOT INSIDE THE SLOPE! -> closestRaycast.distance = " + closestRaycast.distance);
                        //horVel = horVel * (closestRaycast.distance);
                        //vel = new Vector3(horVel.x, vel.y, horVel.z);
                        if (Mathf.Abs(closestRaycast.distance) > 1)
                        {
                    ...
                    if (!ClimbSlope(ref vel, closestRaycast, wallSlide))
                    {
                        collisions.climbJump = true;
                    }
                    //Debug.Log("Finish ClimbSlope = " + vel.ToString("F4") + "; CollisionState = " + collisions.collSt + "; below = " + collisions.below);
                    horVel = new Vector3(vel.x, 0, vel.z);
                    horVel = horVel.normalized * (horVel.magnitude + distanceToSlopeStart);
                    ...
                    Raycast closestCheckForNextSlopeRay = new Raycast(new RaycastHit(), Vector3.zero, float.MaxValue, Vector3.zero, Vector3.zero);
                    for (int k = 0; k < raysPerRow; k++)
                    {
                        horVel = new Vector3(vel.x, 0, vel.z);
                        ...
                        if (Physics.Raycast(rayOrigin, rayDir, out hit, rayLength, collisionMask, QueryTriggerInteraction.Ignore))
                        {
                            ...
                            if ((!wallSlide && slopeAngle != collisions.slopeAngle) || (wallSlide && slopeAngle != collisions.wallSlideSlopeAngle))//NEW SLOPE FOUND
                            {
                                float wallAngle = GetWallAngle(hit.normal);// SignedRelativeAngle(Vector3.forward, hit.normal, Vector3.up);// Vector3.Angle(hit.normal, Vector3.forward);
                                Raycast auxRay = new Raycast(hit, hit.normal, hit.distance - originalRaycast.skinWidth, vel, rayOrigin, true, slopeAngle, wallAngle, originalRaycast.axis,
                                    originalRaycast.row, originalRaycast.column, rows, originalRaycast.skinWidth);
                                if (auxRay.distance < closestCheckForNextSlopeRay.distance)
                                {
                                    closestCheckForNextSlopeRay = auxRay;
                                }
                            }
                        }
                    }
                    if (closestCheckForNextSlopeRay.hit)
                    {
                        if (IsSlopeAngleAWall(closestCheckForNextSlopeRay.slopeAngle))
                        {
                            if (collisions.fcww == FirstCollisionWithWallType.none)
                            {
                                if (!disableAllDebugs) Debug.Log("FCWW NOT TRIGGERED so we do it");
                                collisions.fcww = closestCheckForNextSlopeRay.normal.y < 0 ? FirstCollisionWithWallType.climbingAndFowardWall : closestCheckForNextSlopeRay.normal.y > 0 ?
FirstCollisionWithWallType.climbingAndBackwardsWall : FirstCollisionWithWallType.climbingAndStraightWall;
                                ClimbingAndFoundWall(ref vel, closestRaycast, closestCheckForNextSlopeRay);
                            }
                        }
                        else
                        {
    ...
    || collisions.fcww == FirstCollisionWithWallType.climbingAndStraightWall)
        {
            ...
            switch (collisions.fcww)
            {
                ...
                case FirstCollisionWithWallType.climbingAndFowardWall:
                    collisions.fcwwSlopeAngle = closestWallRaycast.slopeAngle;
        ...
        if (!WallEdgeMultiple(vel, ref closestHorRaycast, rows))
        {
            ...
            if (found > 0)//DOS TIPOS DE MURO ENCONTRADOS
            {
                ...
                if (((closestRayIsRight && cornerAngle >= 0) || (!closestRayIsRight && cornerAngle < 0)) && Mathf.Abs(cornerAngle) != 180)// -- ES UNA ESQUINA CONVEXA -- 
                {
                    ...
                    if (!disableAllDebugs) Debug.Log("WALL EDGE ALL: CHECK IF VALID EDGEVECTOR ANGLE->" + "; closestHorRaycast.axis = " + closestHorRaycast.axis + "; vel.z = "
                        + vel.z + "; vel.x = " + vel.x + "; closestHorRaycast.row = " + closestHorRaycast.row);
                    if (closestHorRaycast.axis == Axis.X && !(Mathf.Abs(vel.z) > Mathf.Abs(vel.x) && ((closestHorRaycast.column == 0 && vel.z > 0) ||
                        (closestHorRaycast.column == columns - 1 && vel.z < 0))))
                    {
                    ...
                    if (!disableAllDebugs) Debug.LogWarning("WALL EDGE ALL: EDGE ANGLE = " + edgeAngle + "; edgeVectorProyectedAngle = " + edgeVectorProyectedAngle +
                        "; edgeVector = " + edgeVector.ToString("F6"));
                    if (!disableAllRays && showWallEdgeRays)
                    {
                    ...
                    int edgeAlgorithm = (edgeVector != Vector3.up && edgeVector != Vector3.down) && edgeVectorProyectedAngle > 30 ? 2 : 1;
                    while (edgeAlgorithm >= 0)
                    {
                        switch (edgeAlgorithm)
                        {
                            ...
                            case 2:
                                ...
                                if (!disableAllDebugs) Debug.Log("WALL EDGE ALL: planeNormal = " + planeNormal.ToString("F4") + "; firstPlaneVector = " + firstPlaneVector.ToString("F4") +
    ...
    Raycast PlaneBetween2Points(Vector3 vel, Raycast closestRaycast, Raycast closestRaycast2)
    {
        ...
        if (!disableAllRays && showWallEdgeRays) DrawPlane(closestRaycast.ray.point, planeNormal, orange, 0.8f);
        #endregion
    ...
    Vector3 GetLastHorVel(Vector3 vel)
    {
        Vector3 horVel = new Vector3(vel.x, 0, vel.z);
        return horVel.magnitude != 0 ? horVel : collisions.lastHorVel;
    }
    ...
    void VerticalCollisions(ref Vector3 vel)
    {
        ...
        if (collisions.closestVerRaycast.hit)//si ha habido una collision vertical
        {
            ...
            if (!peak && VerticalEdge(vel, collisions.closestVerRaycast)) peak = true;
            if (!peak)
            {
                ...
                {
                    value = CollisionState.roof_floor;
                }
            ...
            if (!disableAllDebugs) print("Vertical collisions value post= " + value);
            switch (value)//con que tipo de objeto colisionamos? suelo/cuesta arriba/cuesta abajo
            {
                #region FLOOR/ROOF
                case CollisionState.roof_floor:

                    #region --- CLIMB VS WALL ---
                    if (collisions.collSt == CollisionState.climbing && vel.y > 0)//Subiendo chocamos con un techo
                    {
                        if (!disableAllDebugs) Debug.Log(" collisions.fcww = " + collisions.fcww + "; collisions.fcwwSlopeAngle = " + collisions.fcwwSlopeAngle +
                            "; collisions.closestVerRaycast.slopeAngle = " + collisions.closestVerRaycast.slopeAngle);
                        if (!(collisions.fcww == FirstCollisionWithWallType.climbingAndFowardWall && collisions.fcwwSlopeAngle == collisions.closestVerRaycast.slopeAngle))
                        {
                            ...
                            #region -- CLIMB VS WALL FIRST TIME --
                            ...
                            {
                                if (!disableAllDebugs) Debug.LogError("Climb vs Roof first time: collisions.oldRoofAngle = " + collisions.oldRoofAngle +
                                    "; collisions.closestVerRaycast.slopeAngle = " + collisions.closestVerRaycast.slopeAngle);
                            ...
                            else
                            {
                                ...
                                Vector3 roofWallRayOrigin = collisions.closestHorRaycast.origin + (horVelAux.normalized * collisions.closestHorRaycast.skinWidth);
                                roofWallRayOrigin -= horVelAux.normalized * skinWidth;
                                Vector3 roofWallRayDir = horVelAux.normalized;
                                ...
                                if (roofWall.Raycast(roofWallRay, out roofWallEnter))
                                {
                                    ...
                                    Raycast auxRay = new Raycast(collisions.closestHorRaycast.ray, roofWallNormal, distance, vel, roofWallRayOrigin, true, slopeAngle, wallAngle, collisions.closestHorRaycast.axis, collisions.closestHorRaycast.row,
                                        collisions.closestHorRaycast.column, horizontalRows, skinWidth);
                                    WallSlide(ref vel, auxRay);
                                    collisions.collSt = CollisionState.climbing;
                                    vel.y = (collisions.closestVerRaycast.distance) * directionY;
                                }
                                else if (!disableAllDebugs) Debug.LogError("Error: could not hit roofWall! roofWallRayOrigin = " + roofWallRayOrigin);
                            }
                            #endregion
                        }
                    }
                    ...
                    {
                        if (collisions.collSt == CollisionState.none) collisions.collSt = CollisionState.roof_floor;

                        if (!disableAllDebugs) Debug.Log("Start Colisión con techo o suelo vertical normal; vel = " + vel.ToString("F4"));
                        if (collisions.climbJump)
                        {
                            Vector3 horVelAux = new Vector3(vel.x, 0, vel.z);
                ...
                case CollisionState.descending:
                    ...
                    if (Physics.Raycast(descendRayOrigin, Vector3.down, out descendHit, rayLength, collisionMask, QueryTriggerInteraction.Ignore))
                    {
                        float slopeAngle = GetSlopeAngle(descendHit);
                        if (!disableAllRays) Debug.DrawRay(descendRayOrigin, Vector3.down * rayLength, Color.magenta);

                        if (slopeAngle != collisions.slopeAngle)
                        {
                            vel.y = -(descendHit.distance - skinWidth);
                            collisions.slopeAngle = slopeAngle;
                        }
                    }
                    break;
    ...
    bool VerticalEdge(Vector3 vel, Raycast closestVerRaycast)
    {
        bool success = false;
        CollisionState closestRayVal = CheckSlopeType(GetLastHorVel(vel), closestVerRaycast);
        float slopeAngle1 = closestVerRaycast.slopeAngle;
        if (vel.y < 0 && ((collisions.lastCollSt == CollisionState.climbing && slopeAngle1 != collisions.slopeAngleOld) || collisions.lastCollSt == CollisionState.none
            || collisions.lastCollSt == CollisionState.crossingPeak) && slopeAngle1 != 0)
        {
            if (!disableAllDebugs) Debug.LogError("VERTICAL EDGE STARTED: closestRayVal = " + closestRayVal + "; collisions.lastCollSt = " + collisions.lastCollSt);
            int found = 0;
            float directionY = Mathf.Sign(vel.y);
            Raycast otherFloorRaycast = new Raycast(new RaycastHit(), Vector3.zero, float.MaxValue, Vector3.zero, Vector3.zero);
            //Raycast auxClosestRay = closestVerRaycast;

            Vector3 rayOrigin = closestVerRaycast.origin;
            Vector3 rayDir = Vector3.up * directionY;
            float rayLength = Mathf.Abs(vel.y) + skinWidth + rayExtraLengthPeak;
            int row = closestVerRaycast.row;
            int column = closestVerRaycast.column;

            #region --- Horizontalmente ---
            for (int sentido = -1; sentido < 2 && found == 0; sentido = sentido == -1 ? 1 : 2)
            {
                Vector3 localRayOrigin = rayOrigin;
                if (!disableAllDebugs) Debug.Log("Floor Edge Horizontal: sentido = " + sentido + "; column = " + column + "; (verticalRaysPerRow-1) = " + (verticalRaysPerRow - 1));
                if (!((sentido == -1 && column == 0) || (sentido == 1 && column == verticalRaysPerRow - 1)))
                {
                    for (int k = 1; k < floorEdgeHighPrecissionRays - 1 && found == 0; k++)
                    {
                        ...
                        if (Physics.Raycast(localRayOrigin, rayDir, out hit, rayLength, collisionMask, QueryTriggerInteraction.Ignore))
                        {
                            float slopeAngle = GetSlopeAngle(hit);
            ...
            if (found == 0)
            {
                ...
                {
                    Vector3 localRayOrigin = rayOrigin;
                    if (!disableAllDebugs) Debug.Log("Floor Edge Vertical: sentido = " + sentido + "; row = " + row + "; (verticalRows-1) = " + (verticalRows - 1));

                    if (!((sentido == -1 && row == verticalRows - 1) || (sentido == 1 && row == 0)))
                    {
                        for (int k = 1; k < floorEdgeHighPrecissionRays - 1 && found == 0; k++)
                        {
            ...
            if (!disableAllDebugs) Debug.LogWarning("VERTICAL EDGE: found = " + found);
        }
        if (!disableAllDebugs) Debug.Log("VERTICAL EDGE FINISHED");
        return success;
    ...
    void CalculateRaySpacing()
    {
        ...
        floorEdgeHighPrecissionHorRaySpacing = verticalRaySpacing / (floorEdgeHighPrecissionRays - 1);
        floorEdgeHighPrecissionVerRaySpacing = verticalRowSpacing / (floorEdgeHighPrecissionRays - 1);
