...
public class PlayerHook_Online : MonoBehaviourPun
{
    PlayerMovement_Online myPlayerMov;
    ...
    private void Awake()
    {
        myPlayerMov = GetComponent<PlayerMovement_Online>();
    ...
    public void StartHook()
    {
        if (hookSt == HookState.ready)
        {
            ...
            Debug.Log("Juan: Comentada la linea 208 de PlayerHook_Online, myHook.KonoAwake() usa una variable de tipo 'playermovement' y no una variable de tipo 'playermovement_online'");
    ...
    public void HookObject(Transform item)
    {
        if (canHookSomething)
        {
            if (item.tag == "Flag")
            {
                ...
                Debug.Log("Juan: Comentada la línea 237 a 242 de PlayerHook_Online, flag.HookFlag() usa una variable de tipo playermovement y no playermovement_online");
    ...
    void HandinObject()
    {
        if (objectHooked)
        {
            if (hookedObject.tag == "Flag")
            {
                ...
                Debug.Log("Juan: Comentada la línea 354 de PlayerHook_Online encargada de coger la bandera, la función flag.PickupFlag() usa una entrada de tipo 'playermovement' y no 'playermovement_online'");
        ...
        if (enemyHooked)
        {
            if (enemy.haveFlag)
            {
                Debug.Log("Juan: Comentada la línea 369 de PlayerHook_Online encargada de robar la bandera, la función enemy.flag.StealFlag() usa una entrada de tipo 'playermovement' y no 'playermovement_online'");
