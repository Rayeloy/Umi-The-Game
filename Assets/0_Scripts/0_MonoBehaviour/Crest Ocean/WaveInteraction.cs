...
namespace Crest
{
    ...
    public class WaveInteraction : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            myPlayerMovement = GetComponentInParent<PlayerMovementCMF>();
            myPlayerBody = myPlayerMovement.myPlayerBody;
            myPlayerRB = myPlayerMovement.GetComponent<Rigidbody>();
        ...
        void LateUpdate()
        {
            ...
            var disp = myPlayerBody.displacementToObject;
            //Debug.Log("playerDisp = " + disp);

            //Debug.Log("Velocity = " + myPlayerRB.velocity);
            transform.position = transform.parent.TransformPoint(_localOffset) - disp + _velocityPositionOffset * myPlayerRB.velocity;
            ...
            {
                //Debug.Log("width = " + myPlayerBody.bodyWidth);
                _sampleFlowHelper.Init(transform.position, myPlayerBody.bodyWidth);
            ...
            //Debug.Log("Floating? " + (myPlayerMovement.vertMovSt == VerticalMovementState.FloatingInWater));
            float weight = myPlayerMovement.vertMovSt == VerticalMovementState.FloatingInWater ? _weight / simsActive : 0f;
